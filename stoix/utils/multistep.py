...
def batch_truncated_generalized_advantage_estimation(
    ...
    #jax.debug.print(f'target_values {target_values.shape} advantage_t {advantage_t.shape}')
...
def batch_truncated_generalized_advantage_estimation2(
    r_t: chex.Array,
    discount_t: chex.Array,
    lambda_: Union[chex.Array, chex.Scalar],
    adv: chex.Array,
    ...
    if not time_major:
        batch_size = r_t.shape[0]
        r_t, discount_t, adv, truncation_mask = jax.tree_util.tree_map(
            lambda x: jnp.swapaxes(x, 0, 1), (r_t, discount_t, adv, truncation_mask)
        )
    else:
        batch_size = r_t.shape[1]

    chex.assert_type([r_t, adv, discount_t, truncation_mask], float)
